...
def is_group(update, context):
    ...
    if choix == 'Groupe':
        ...
        if groups_in !=[]:
            ...
            query.message.reply_text('Erreur : Vous ne faîtes parti de aucun groupe')
            return ConversationHandler.END
...
def choose_add_tags(update, context):
    ...
    if choix == 'Oui':
        query.answer()
        query.edit_message_text(text=f"Entrez le tag a ajouté")
        if type == 'text':
            return ADD_TAGS
        elif type == 'audio':
        ...
        query.edit_message_text(text=f"Entrez un titre")
        if type == 'text':
            return TITLE_INPUT
        elif type == 'audio':
...
def define_title(update, context):
    ...
    cursor.execute(insert_query_doc)
    connection.commit()
    update.message.reply_text(f'Votre note a bien été enregistrée.')
